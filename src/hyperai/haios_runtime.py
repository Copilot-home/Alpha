...
class PolicyEngine:
    ...
    def validate_action(self, action_type: str, action_payload: Dict) -> Dict[str, Any]:
        ...
        # Composite score
        composite = sum(
            pillars[p] * self.invariants.PILLARS[p]["weight"] for p in pillars
        )
...
class HAIOS:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "uptime": (
                datetime.utcnow() - datetime.fromisoformat(self.state["boot_time"])
            ).total_seconds(),
            "actions_executed": self.state["actions_executed"],
            "actions_blocked": self.state["actions_blocked"],
            "success_rate": (
                self.state["actions_executed"]
                / (self.state["actions_executed"] + self.state["actions_blocked"])
                if (self.state["actions_executed"] + self.state["actions_blocked"]) > 0
                else 0
            ),
            "k_state": self.state["current_k_state"],
...
def demo():
    ...
    with open(haios.audit_log.log_file, "r") as f:
        ...
        for line in lines[-5:]:
            ...
            print(
                f"   [{entry['event_type']}] {entry['action_type']}: {entry['execution_status']}"
            )
    print()
